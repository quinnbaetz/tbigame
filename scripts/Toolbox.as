package tbigame.scripts {
	...
	public class Toolbox extends MovieClip
	{
		...
		function Toolbox(theStage, scope){
			...
			pad = new TabletLogic(theStage, scope, this);
			//var tablet = new Tablet();
			
			/***** ADDED TO TABLET CLASS
			tablet.x = padX;
			tablet.y = padY;
			tablet.scaleX = scale;
			tablet.scaleY = scale;
			...
			*/
			...
			//******ADDED TO TABLET CLASS***********
			//this.scope.addCache(pad, this.theStage, "Tablet");
			//chart = tablet;
			...
			menuScreen.muteButton.addEventListener(MouseEvent.CLICK, toggleSound);
			//----
			
			
			clock.myAddEventListener(MouseEvent.MOUSE_DOWN, pad.padToggle);
			index.addEventListener(MouseEvent.CLICK, pad.padToggle);
		...
		public function showPad(){
			bringForward();
			//hideMenu();
			pad.padToggle(null, true, false);
		}
		public function hidePad(){
			bringForward();
			pad.padToggle(null, false, true);
		}
		/**
		...
		*/
		...
		public function bringForward(){
			...
			myStage.setChildIndex(myStage.getChildByName(pad.pad.name), myStage.numChildren-1);
		...
		public function toggleSound(e=null):void {
